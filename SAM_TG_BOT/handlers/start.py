...
async def cmd_start(message: Message):
    ...
    if message.from_user.id not in TEACHERS_IDS:
        ...
        await message.answer("Assalomu alaykum ustoz ozingizga ovoz bera olmaysiz.")
...
async def its_user_answer(message: Message):
    ...
    if message.from_user.id not in TEACHERS_IDS:
        ...
        if answered_questions[user_id] <= 6:
            await message.react([types.ReactionTypeEmoji(emoji='👌')])
            ...
            await message.answer(f"<b>{queastions[answered_questions[user_id]-1]}</b>", parse_mode="html", reply_markup=reply_kb_3)
            answered_questions[user_id] += 1

        elif answered_questions[user_id] == 7:
